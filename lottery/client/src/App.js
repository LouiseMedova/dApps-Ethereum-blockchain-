...
function App() {
  ...
  async function updateBet() {
    ...
    setBet({count: bet[0], size: bet[1], admin: bet[2], state: parseInt(bet[3]), winner: bet[4]});
  ...
  async function createBet(e) {
    e.preventDefault();
    const betSize = e.target.elements[0].value;
    const betCount = e.target.elements[1].value;
    await contract.methods
        .createBet(betSize, betCount)
